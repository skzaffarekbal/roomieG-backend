...
profileRouter.post('/get-presigned-url', userAuth, async (req, res) => {
  try {
    ...
    if (!['png', 'jpg', 'jpeg'].includes(mimeType)) throw new Error('Invalid mime type.');

    const key = `${user._id.toString()}.${mimeType}`;
    const blurKey = `${user._id.toString()}_F2.${mimeType}`;
    ...
    return res.status(200).json({
      ...
      uploadUrl: url,
      uploadBlurUrl: blurUrl,
      finalFileName: key,
      finalBlurFileName: blurKey,
    });
  } catch (error) {
...
profileRouter.patch('/profile/photo', userAuth, async (req, res) => {
  try {
    ...
    user.photo.exactPhoto = process.env.AWS_IMAGE_URL + exactPhoto;
    user.photo.blurPhoto = process.env.AWS_IMAGE_URL + blurPhoto;
